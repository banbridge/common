...
var CmdServer = &cobra.Command{
	...
	Long:  "Generate the proto Server implementations. Example: yakt proto server api/xxx.proto --target-dir=internal/service",
	Run:   run,
}
...
func run(cmd *cobra.Command, args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "Please specify the proto file. Example: yakt proto server api/xxx.proto")
		return
	}
	...
	for _, s := range res {
		...
		if _, err := os.Stat(to); !os.IsNotExist(err) {
			fmt.Fprintf(os.Stderr, "%s alreay exsits: %s\n", s.Service, to)
			continue
		}
		b, err := s.excute()
		if err != nil {
